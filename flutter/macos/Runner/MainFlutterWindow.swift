...
class MainFlutterWindow: NSWindow {
    override func awakeFromNib() {
        shopremote2_core_main();
        let flutterViewController = FlutterViewController.init()
        let windowFrame = self.frame
        self.contentViewController = flutterViewController
        self.setFrame(windowFrame, display: true)
        // register self method handler
        let registrar = flutterViewController.registrar(forPlugin: "ShopRemote2Plugin")
        ...
        FlutterMultiWindowPlugin.setOnWindowCreatedCallback { controller in
            ...
            self.setMethodHandler(registrar: controller.registrar(forPlugin: "ShopRemote2Plugin"))
    ...
    private func enableNativeRelativeMouseMode(channel: FlutterMethodChannel) -> Bool {
        ...
        if result != CGError.success {
            NSLog("[ShopRemote2] Failed to dissociate mouse from cursor position: %d", result.rawValue)
            return false
        }
        ...
        if state.eventMonitor == nil {
            NSLog("[ShopRemote2] Failed to create event monitor for relative mouse mode")
    ...
    private func disableNativeRelativeMouseMode() {
        ...
        if result != CGError.success {
            NSLog("[ShopRemote2] Failed to re-associate mouse with cursor position: %d, scheduling retry...", result.rawValue)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                if retryResult != CGError.success {
                    NSLog("[ShopRemote2] Retry failed to re-associate mouse: %d. Cursor may remain locked.", retryResult.rawValue)
                }
            }
        }
    }
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "org.shopremote2.shopremote2/host", binaryMessenger: registrar.messenger)
