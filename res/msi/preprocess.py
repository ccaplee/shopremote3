...
g_arpsystemcomponent = {
    ...
    "Contact": {
        "msi": "ARPCONTACT",
        "v": "https://github.com/ccaplee/shopremote3",
    },
    "HelpLink": {
        "msi": "ARPHELPLINK",
        "v": "https://github.com/ccaplee/shopremote3/issues/",
    },
    "ReadMe": {
        "msi": "ARPREADME",
        "v": "https://github.com/ccaplee/shopremote3",
...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="ShopRemote3", help="The app name."
    )
    parser.add_argument(
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "Package/Components/ShopRemote3.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("ShopRemote3", app_name)
...
def replace_app_name_in_custom_actions(app_name):
    ...
    for file_path in chain(custion_actions_dir.glob("*.cpp"), custion_actions_dir.glob("*.h")):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            line = re.sub(r"\bShopRemote3\b", app_name, line)
            line = line.replace(f"{app_name} v4 Printer Driver", "ShopRemote3 v4 Printer Driver")
            lines[i] = line
        with open(file_path, "w", encoding="utf-8") as f:
            f.writelines(lines)
...
def update_license_file(app_name):
    if app_name == "ShopRemote3":
        return
    license_file = Path(sys.argv[0]).parent.joinpath("Package/License.rtf")
    with open(license_file, "r", encoding="utf-8") as f:
        license_content = f.read()
    license_content = license_content.replace("website shopremote3 and other ", "")
    license_content = license_content.replace("ShopRemote3", app_name)
