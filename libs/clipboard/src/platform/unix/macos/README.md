...
1. Use a temporary file `/tmp/shopremote2_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.shopremote2_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.shopremote2_*`.
